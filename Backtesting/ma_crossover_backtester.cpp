...
public:
    MAcrossoverStrategy(int shortPeriod, int longPeriod, double initialCapital)
    ...
    void exportTradeResults(const std::string& filename) {
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: " << argv[0] << " <csv_filename> [short_period=10] [long_period=30] [initial_capital=10000]" << std::endl;
        return 1;
    }
    
    std::string filename = argv[1];
    int shortPeriod = (argc > 2) ? std::stoi(argv[2]) : 10;
    int longPeriod = (argc > 3) ? std::stoi(argv[3]) : 30;
    double initialCapital = (argc > 4) ? std::stod(argv[4]) : 10000.0;
    
    MAcrossoverStrategy strategy(shortPeriod, longPeriod, initialCapital);
    
    if (!strategy.loadCSVData(filename)) {
        return 1;
    }
